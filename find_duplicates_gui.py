...
class DuplicateFinderApp:
    ...
    def _build_ui(self):
        ...
        Label(
            footer_frame,
            text=f"Created by Fred R Phillips for family and friends  |  v{VERSION}",
            font=("Arial", 9), fg="#555555", anchor=W,
        ).pack(side=LEFT)
        Label(
            footer_frame,
            text=f"© {time.strftime('%Y')} Fred R Phillips. All rights reserved.",
            font=("Arial", 9), fg="#555555", anchor=E,
        ).pack(side=RIGHT)
    ...
    def _do_scan(self, folder):
        ...
        lines.append("")
        lines.append(f"Generated by Duplicate Photo Finder v{VERSION}")
        lines.append(f"Created by Fred R Phillips for family and friends")
        lines.append(f"Copyright {time.strftime('%Y')} Fred R Phillips. All rights reserved.")
        lines.append("")
        ...
        lines.append("Please read this guide before taking any action.")
        lines.append("")
        lines.append(f"**Section 1 — Exact duplicates ({len(real_exact)} groups, {dup_count} extra copies)**")
        lines.append("Byte-for-byte identical copies of the same file.")
        lines.append("SAFE TO DELETE — keep one, delete the rest.")
        lines.append("")
        lines.append(f"**Section 2 — Same photo, different file ({len(confirmed)} groups, {perc_count} extra copies)**")
        lines.append("Same image saved at different size, quality, or format.")
        lines.append("SAFE TO DELETE — keep the highest quality version (usually the largest file).")
        lines.append("")
        lines.append(f"**Section 3 — Similar but NOT identical ({len(likely_different)} groups, {sim_count} photos)**")
        lines.append("Different photos that look alike — burst shots, retakes, similar compositions.")
        lines.append("DO NOT DELETE — these are different photos. Listed for your awareness only.")
        lines.append("")
        lines.append(f"**Bottom line:** Sections 1 and 2 found **{safe_delete} files** that can be safely removed.")
        lines.append(f"Section 3 photos are NOT duplicates — review them manually if you wish.")
        lines.append("")
        lines.append("---")
        lines.append("")
